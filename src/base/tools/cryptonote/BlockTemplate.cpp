...
void xmrig::BlockTemplate::calculateMerkleTreeHash(uint32_t index)
{
    ...
    const size_t count = (m_hashes.size() / kHashSize) + ((m_coin == Coin::SALVIUM) ? 1 : 0);
...
bool xmrig::BlockTemplate::parse(bool hashes)
{
    ...
    if (hashes) {
        ...
        m_hashes.clear();
        m_hashes.resize((coinbase_tx_index + m_numHashes + ((m_coin == Coin::SALVIUM) ? 2 : 1)) * kHashSize);
        ...
        if (m_coin == Coin::SALVIUM) {
            calculateMinerTxHash(blob(PROTOCOL_TX_PREFIX_OFFSET), blob(PROTOCOL_TX_PREFIX_END_OFFSET), m_hashes.data() + kHashSize); // Protocol TX
        }

        for (uint64_t i = ((m_coin == Coin::SALVIUM) ? 0 : 1); i <= m_numHashes + ((m_coin == Coin::SALVIUM) ? 1 : 0); ++i) {
            Span h;
            ar(h, kHashSize);
            memcpy(data + (i + ((m_coin == Coin::SALVIUM) ? 2 : 0)) * kHashSize, h.data(), kHashSize);
        }
